...
async function testAdvancedKPIs() {
    try {
        console.log('🧪 Probando sistema avanzado de KPIs...');
        
        // Obtener el vehículo de prueba
        const vehicle = await prisma.vehicle.findFirst({
            where: { name: 'Vehículo Test 2' }
        });
        
        if (!vehicle) {
            console.log('❌ No se encontró el vehículo de prueba');
            return;
        }
        
        console.log(`🚗 Vehículo: ${vehicle.name} (${vehicle.id})`);
        
        // Obtener una sesión reciente
        ...
        console.log(`📅 Fecha de sesión: ${sessionDate.toISOString().slice(0, 10)}`);
        
        // Simular cálculo avanzado de KPIs
        ...
        if (gpsPoints.length > 1) {
            for (let i = 0; i < Math.min(20, gpsPoints.length - 1); i++) {
        ...
        console.log('\n✅ Sistema avanzado de KPIs funcionando correctamente');
