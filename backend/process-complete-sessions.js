...
async function processCompleteSessions() {
    try {
        console.log('🧠 PROCESANDO SESIONES COMPLETAS CON TODOS LOS TIPOS DE ARCHIVOS...');
            ...
            for (const [date, files] of sessionsByDate) {
                ...
                if (files.estabilidad.length > 0) {
                    ...
                    if (sessionId) {
                        ...
                        // Procesar archivos de ESTABILIDAD (principales)
                        if (files.estabilidad.length > 0) {
                            console.log(`    🔄 Procesando archivos de estabilidad...`);
                            for (const file of files.estabilidad.slice(0, 2)) { // Primeros 2 archivos
                        ...
                        if (files.can.length > 0) {
                            console.log(`    🔄 Procesando archivos CAN...`);
                            for (const file of files.can.slice(0, 2)) { // Primeros 2 archivos
                        ...
                        if (files.gps.length > 0) {
                            console.log(`    🔄 Procesando archivos GPS...`);
                            for (const file of files.gps.slice(0, 2)) { // Primeros 2 archivos
                        ...
                        if (files.rotativo.length > 0) {
                            console.log(`    🔄 Procesando archivos ROTATIVO...`);
                            for (const file of files.rotativo.slice(0, 2)) { // Primeros 2 archivos
...
async function processGPSFile(filePath, sessionId, fileName) {
    try {
        ...
        const dataLines = lines.slice(2); // Saltar header y data header
        const measurements = [];
        
        const baseTime = Date.now();
        
        for (let i = 0; i < Math.min(40, dataLines.length); i++) {
            const line = dataLines[i];
            
            // Filtrar líneas que no contengan datos GPS válidos
